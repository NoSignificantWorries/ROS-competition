        ...
        if name.startswith("fork"):
            ...
            for wname, wdata in points.items():
                ...
                for subname, subpoints in wdata.items():
                    res_points["points"][name][wname][subname] = []
                    for x, y in subpoints:
                        xnew = new_w[0] + (x - offset_x) * kx
                        ynew = new_h[0] + (y - offset_y) * ky
                        res_points["points"][name][wname][subname].append((xnew, ynew))
        else:
            for x, y in points:
                xnew = new_w[0] + (x - offset_x) * kx
                ynew = new_h[0] + (y - offset_y) * ky
                res_points["points"][name].append((xnew, ynew))


    print(res_points["points"]["start"][0])
